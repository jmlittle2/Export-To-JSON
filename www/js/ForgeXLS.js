...
var ForgeXLS = {
    ...
    downloadJSON: function (urn, fileName, token, status, fileType) {
        ...
        this.prepareTables(urn, token, function (tables) {
            if (status) status(false, 'Building JSON file...');
            var jsonObject;
            jQuery.each(tables, function (name, table) {
                if (name.indexOf('<') == -1) { // skip tables starting with <
                    //var ws = ForgeXLS.sheetFromTable(table);
                    //var jsonObject=JSON.stringify(XLSX.utils.sheet_to_json(ws), null, '\t');
                    if (jsonObject != null) {
                        jsonObject = jsonObject.concat(JSON.stringify(table, null, '\t'));
                    }
                    else {
                        jsonObject = JSON.stringify(table, null, '\t');
                    }
                }
            });
            saveAs(new Blob([jsonObject], { type: "application/octet-stream" }), fileName);
        ...
        this.Utility.getMetadata(urn, token, function (metadata) {
            ...
            ForgeXLS.Utility.getHierarchy(urn, guid, token, function (hierarchy) {
                ForgeXLS.Utility.getProperties(urn, guid, token, function (properties) {
                    callback(ForgeXLS.prepareRawData(hierarchy, properties, format));
                });
            });
        });
    },
    ...
    prepareRawData: function (hierarchy, properties) {
        var tables = {};
        //var model = properties.data.collection[0];
        var rows = [];
        var columns;
        for (var item of properties.data.collection) {
            if (item.name == 'Model') {
                tables = {};
                columns = ForgeXLS.getProperties(item.objectid, properties);
                rows.push(columns);
                tables['Key Building Charachteristics'] = rows;
                break;
            }
        }
        rows = [];
        ...
        //         columns = ForgeXLS.getProperties(objectid, properties);
        ...
        // getting model obj
        var modelData={};
        modelData = ForgeXLS.getModelObject(hierarchy.data.objects);

        if (hierarchy.data.objects[0].name == 'Model') {
            modelData = hierarchy.data.objects[0];
        }
        else {
            modelData = hierarchy.data.objects[0].objects[0];
        }
        modelData.objects.forEach(function (category) {
            var idsOnCategory = [];
            ForgeXLS.getAllElementsOnCategory(idsOnCategory, category.objects);
            rows = [];
            idsOnCategory.forEach(function (objectid) {
    ...
    getModelObject: function (objects) {
        // if (Array.isArray(objects)) {
        //     objects.forEach(function (object) {
        //         if (object.name == 'Model') {
        //             return object;
        //         }
        //         else if (object.objects !== null) {
        //             ForgeXLS.getModelObject(object.objects);
        //         }
        //     });
        // }
        if (Array.isArray(objects)) {
            for (var i = 0; i < objects.length; i++) {
                if (objects[i].name == 'Model') {
                    return objects[i];
                }
                else if (objects[i].objects !== null) {
                    ForgeXLS.getModelObject(objects[i].objects);
                }
            }
        }
    },
    ...
    getProperties: function (id, objCollection) {
        ...
        objCollection.data.collection.forEach(function (obj) {
            if (obj.objectid != id || obj.externalId == '') return;
            if (obj.name == 'Model') {
                data['Category'] = 'Model';
                ...
                if (obj.properties.hasOwnProperty(propGroup)) {
                    for (var propName in obj.properties[propGroup]) {
                        if (obj.properties[propGroup].hasOwnProperty(propName) && Array.isArray(obj.properties[propGroup][propName])) {
                            //data[propGroup + ':' + propName] = obj.properties[propGroup][propName][0];
                            data[propName] = obj.properties[propGroup][propName][0];

                        }
                        else if (obj.properties[propGroup].hasOwnProperty(propName) && !Array.isArray(obj.properties[propGroup][propName])) {
                            //data[propGroup + ':' + propName] = obj.properties[propGroup][propName];
                            data[propName] = obj.properties[propGroup][propName];
                        }
                    }
                }
            }
            ...
            // for (var propGroup in obj.properties) {
            //     if (propGroup.indexOf('__') > -1) break;
            //     if (obj.properties.hasOwnProperty(propGroup) && format == 'xls') {
            //         for (var propName in obj.properties[propGroup]) {
            //             if (obj.properties[propGroup].hasOwnProperty(propName) && !Array.isArray(obj.properties[propGroup][propName]) && obj.name !== 'Model' && propName.contains('PLGS'))
            //                 data[propGroup + ':' + propName] = obj.properties[propGroup][propName];
            //             else if (obj.properties[propGroup].hasOwnProperty(propName) && !Array.isArray(obj.properties[propGroup][propName]) && obj.name == 'Model' && propName.contains('PLGS'))
            //                 data[propGroup + ':' + propName] = obj.properties[propGroup][propName];
            //         }
            //     }
            //     else if (obj.properties.hasOwnProperty(propGroup) && format == 'json') {
            //         for (var propName in obj.properties[propGroup]) {
            //             if (obj.properties[propGroup].hasOwnProperty(propName) && !Array.isArray(obj.properties[propGroup][propName]) && obj.name !== 'Model')
            //                 data[propGroup + ':' + propName] = obj.properties[propGroup][propName];
            //             else if (obj.properties[propGroup].hasOwnProperty(propName) && !Array.isArray(obj.properties[propGroup][propName]) && obj.name == 'Model' && propName.contains('PLGS'))
            //                 data[propGroup + ':' + propName] = obj.properties[propGroup][propName];
            //         }
